...
const char *HelpMessage = R"(
    ...
    -random   随机生成数据
              ...
              用例:
                   [32msortui -random 0 99 40 [0m 生成 40 个 0 ~ 99 的随机数据
    
    -file     文件生成数据
              用法:
                   [33msortui -file <string>[0m
              用例:
                   [32msortui -file [36mtest.txt[0m  用[36mtest.txt[0m来生成数据
              [31m注意:[0m
                   我是直接用[34mifstream[0m读取的, 所以第一个数据是数据长度
                   数字与数字之间只能用空白符分隔
...
----以下选项只在no-ui模式下生效----
    ...
    -speed    控制排序速度
              用法:
                   [33msortui -r 0 99 40 -speed <float> [0m

    -method   使用的排序方法
              用法:
                  [33msortui -f foo.bar -method <string> [0m
              可用方法:
